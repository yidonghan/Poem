...
class Image2360PanoramaImagePipeline(DiffusionPipeline):
    ...
    def save_cube_faces(self, panorama_img, output_path_prefix):
        """
        将全景图像转换为立方体六个面并保存
        
        Args:
            panorama_img (PIL.Image): 全景图像
            output_path_prefix (str): 输出文件路径前缀
        """
        # 将PIL图像转换为numpy数组
        panorama_np = np.array(panorama_img)
        
        # 使用py360convert将等距柱状投影转换为立方体六个面
        cube_faces = py360convert.e2c(panorama_np, face_w=1024, mode='bilinear')
        ...
        for i, face_name in enumerate(face_names):
            face_img = Image.fromarray(cube_faces[i])
            face_path = f"{output_path_prefix}_{face_name}.png"
            face_img.save(face_path)
            print(f"Saved {face_name} face to {face_path}")
    ...
    def __call__(self, inputs: Dict[str, Any],
        ...
        # 在返回之前，如果需要保存立方体面
        if inputs.get('save_cube_faces', False):
            output_path_prefix = inputs.get('output_path_prefix', 'cube_face')
            self.save_cube_faces(output_img, output_path_prefix)
